...
DBOXRunActionMessenger::DBOXRunActionMessenger(DBOXRunAction* runAction)
:   G4UImessenger(), 
    fDBOXRunAction(runAction),
    fDirCMD(nullptr), 
...
DBOXRunActionMessenger::~DBOXRunActionMessenger() {
	  delete fEdepHistogramCMD;
    delete fDirCMD;
}


void DBOXRunActionMessenger::SetNewValue(G4UIcommand* command, G4String newValue) {
...
//        G4cout << fname << " " << minEnergy << " " << maxEnergy << " " << numBins << G4endl;
        fDBOXRunAction->SetEdepHistogram(fname, minEnergy*unit, maxEnergy*unit,
                                         numBins);
    }
}
