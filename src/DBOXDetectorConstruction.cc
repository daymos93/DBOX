...
//#include "DBOXDetectorMessenger.hh" //header para el  DetectorConstruction user control
...
// CTR
DBOXDetectorConstruction::DBOXDetectorConstruction()
...
//    fDetMessenger    = new DBOXDetectorMessenger(this); //detector messenger pointer: to set the target thickness
}

// DTR
DBOXDetectorConstruction::~DBOXDetectorConstruction() {
//    delete fDetMessenger;  // The detector messenger pointer: to set the target thickness
}


void DBOXDetectorConstruction::SetTargetMaterial(const G4String& matName) {
    // try to find the material in the NIST DB
    fTargetMaterial = G4NistManager::Instance()->FindOrBuildMaterial(matName);
    if (!fTargetMaterial) {
      G4cerr<< "\n **** ERROR in DBOXDetectorConstruction::SetTargetMaterial() \n"
...
//
G4VPhysicalVolume* DBOXDetectorConstruction::Construct() {
	// I. CREATE MATERIALS:
