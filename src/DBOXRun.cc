...
DBOXRun::DBOXRun(DBOXDetectorConstruction* det, DBOXPrimaryGeneratorAction* prim)
:   G4Run(),
    fDBOXDetector(det),
    fDBOXPrimary(prim) {
    // init data members
    ...
    // /DBOXApp/runAction/edepHisto ... see DBOXRunActionMessenger) and will be
    // updated in DBOXRunAction::BeginOfRunAction method. Invoking this UI
...
// local run (DBOXRun) pointers as argument. Therefore, here you define how to
// merge each thread local Run global data structures (all thread local Run-s
// will be merged to the master Run data). 
void DBOXRun::Merge(const G4Run* run) {
	// Cast the (theard local) G4Run (base) pointer to DBOXRun pointer (it will
	// be DBOXRun since the RunManager will call DBOXRunAction::GenerateRun
	// interface method to generate the Run-s)
	const DBOXRun* localRun = static_cast<const DBOXRun*>(run);
	//
	// Then you can define how to sum up the individual DBOXRun data members
...
void  DBOXRun::EndOfRunSummary() {
    ...
    //primInfo += std::to_string( fDBOXPrimary->GetParticleEnergy() ) + " [MeV] ";
    //primInfo += fDBOXPrimary->GetParticleName() + "\n";

    G4cout<< "\n\n" << G4endl;
    G4cout<< "\n =================================  Run Summary ================================= \n"
          << " Target thickness    : " << fDBOXDetector->GetTargetThickness()/CLHEP::cm << " [cm] \n"
          << " Target material info: " << "                                 \n"
          <<  fDBOXDetector->GetTargetMaterial() << "\n"
